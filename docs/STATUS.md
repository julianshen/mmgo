...
| 🚧 | 26. Sankey | #166, #183 | ~92% / ~93% | PR1: `sankey` alias, frontmatter `title:`, accTitle / accDescr inline with CSV rows. PR2: `LinkColor` (Source / Target / Gradient / Hex) with per-flow `<linearGradient>` defs in Gradient mode, `ShowValues *bool` toggle, `Prefix` / `Suffix` value wrapping, `NodeAlignment` enum stub (Justify implemented; Right / Center / Left fall through pending layout rewrite). PR3 deferred (full NodeAlignment layout, width / height config, %%{init}%% for sankey block) |
...
| 🚧 | 29. C4 | #175, #186 | ~92% / ~91% | Phase 1: 14 new element kinds covering every queue / `_Ext` / `Deployment_Node` / `Node_*` keyword, long-form `Rel_Up`/`Down`/`Left`/`Right`, `BiRel` with `marker-start` + `marker-end`, queue stadium + DB cylinder + dashed `Deployment_Node` shapes, `accTitle` / `accDescr`. Phase 2: `Boundary(...) { ... }` block scoping with stack-based parser (Generic / System / Enterprise / Container kinds), nested boundaries, viewport adjustment for boundary frames extending past layout bounds, `TypeHint` honoured on generic Boundary kind. Phases 3–4 deferred (named-arg `$descr=` / `$tags=` / `$link=` / `$sprite=` / `RelIndex`, `UpdateElementStyle` / `UpdateRelStyle` / `UpdateLayoutConfig`, `LAYOUT_*` directives, legend). |
| 🚧 | 30. Kanban | #176, #181 | ~92% / ~91% | Phase 1: frontmatter `title:` + `ticketBaseUrl:`, `accTitle` / `accDescr` (single + block), `<title>` / `<desc>` SVG emission + title caption above columns. Phase 2: priority-driven 4-px left-edge stripe (Very High / High / Low / Very Low palette), `<a href>` ticket-link wrap with `#TICKET#` substitution. Phase 3 deferred (`classDef` / `class` / `:::class` styling, `%%{init}%%`, `<br/>` / markdown bold in cards, accurate text width measurement) |
| 🚧 | 31. GitGraph | #178, #182 | ~92% / ~89% | Phase 1: `cherry-pick id: "..." [tag] [parent]` with `GitCommitCherryPick` glyph, `switch` alias, `commit msg: "..."`, `branch <name> order: N`, quoted names. Phase 2: frontmatter `title:` + `mainBranchName:`, `accTitle` / `accDescr` (single + block), `<title>` / `<desc>` SVG + title caption. Phase 3 deferred (`%%{init: gitGraph: ...}%%` directive plumbing, `mainBranchOrder` / `showBranches` / `showCommitLabel` / `rotateCommitLabel` / `parallelCommits` toggles, named theme resolution, TB / BT direction rendering, lane ordering by `BranchOrder` + `mainBranchOrder`, validation pass) |
| 🚧 | 32. XY chart | #179, #189 | ~95% / ~91% | Phase A: stable `xychart` keyword alongside legacy `xychart-beta`, frontmatter `title:`, `accTitle` / `accDescr`, `<title>` / `<desc>` SVG emission. Phase B: every documented `themeVariables.xyChart.*` key (12 surfaces + aggregate fallbacks that rebroadcast); full Config covering width/height/title font + every per-axis surface (showLabel / labelFontSize / labelPadding / showTitle / titleFontSize / titlePadding / showTick / tickLength / tickWidth / showAxisLine / axisLineWidth + showDataLabel / showDataLabelOutsideBar / chartOrientation); horizontal layout with correct AST-aligned axis swap; continuous numeric X-axis (`x-axis title min --> max`); data labels (inside vs outside-bar); `BoolPtr` helper for tri-state Show* flags; tickRow unification across all three axis directions. Negative-bar baseline (#190) and renderSeries vertical/horizontal unification (#192) tracked as follow-ups. |
