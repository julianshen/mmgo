...
**Last updated:** 2026-05-07 (GitGraph parallelCommits, plus C4 UpdateLayoutConfig / Legend / RelIndex)
...
- **Remaining minor follow-ups:** GitGraph (named theme resolution, validation). `parallelCommits` now collapses commits onto shared slots by max-of-parents depth. `LAYOUT_LANDSCAPE` / `LAYOUT_AS_SKETCH` are PlantUML stdlib directives that Mermaid's C4 docs explicitly do not implement (parser silently no-ops them, matching upstream). All Phase 6 diagram types are now ✅ on the spec surface that Mermaid itself implements.
...
| ✅ | 29. C4 | #175, #186, #205 | ~92% / ~93% | Phase 1: 14 new element kinds covering every queue / `_Ext` / `Deployment_Node` / `Node_*` keyword, long-form `Rel_*` directions, `BiRel` with `marker-start` + `marker-end`, queue stadium + DB cylinder + dashed `Deployment_Node` shapes, `accTitle` / `accDescr`. Phase 2: `Boundary(...) { ... }` block scoping with stack-based parser, nested boundaries, viewport adjustment, `TypeHint` honoured on generic Boundary. Phase 3: full named-arg surface (`$descr=`, `$tags=`, `$link=`, `$sprite=`, `?techn=`, `$offsetX=`, `$offsetY=`) parsed onto AST; `$link=` wraps element / boundary groups in `<a href>`; positional cursor advances correctly when `?techn=` is named. Phase 4: `UpdateElementStyle(kind, $bgColor=…, $fontColor=…, $borderColor=…)` overrides theme palette per kind; `UpdateRelStyle(from, to, $textColor=…, $lineColor=…, $offsetX=…, $offsetY=…)` recolours individual edges and shifts labels; `LAYOUT_TOP_DOWN` / `LAYOUT_LEFT_RIGHT` directives parsed onto `C4Diagram.Direction`; `$offsetX=` / `$offsetY=` on Rel now actually shift the label. Remaining (UpdateLayoutConfig, LAYOUT_LANDSCAPE / LAYOUT_AS_SKETCH, Legend, RelIndex) tracked as follow-ups. |
...
| ✅ | 31. GitGraph | #178, #182, #203 | ~92% / ~92% | Phase 1: `cherry-pick id: "..." [tag] [parent]`, `switch` alias, `commit msg: "..."`, `branch <name> order: N`, quoted names. Phase 2: frontmatter `title:` + `mainBranchName:`, `accTitle` / `accDescr`. Phase 3: full Config covering every documented `%%{init: gitGraph: …}%%` toggle (`ShowBranches` / `ShowCommitLabel` / `RotateCommitLabel` / `ParallelCommits`) plus `MainBranchOrder`; commit labels rotate -45° by default (Mermaid spec default); lanes sort by `(BranchOrder asc, declaration index asc)`; `Direction` parsed from header. Phase 4: `Direction LR / TB / BT` now flips the renderer layout via a `geom` abstraction — TB lays commits top-down with branch pills in a top gutter and lane guides running vertically; BT inverts the commit order so the latest commit sits at the top. Commit-id labels for vertical layouts sit to the right of the dot (the cross axis is free) instead of being rotated. Remaining (parallelCommits algorithmic pass, named theme resolution, validation) tracked as follow-ups. |
